var CosmosDBSQLAPIDataSource = (function () {
    function CosmosDBSQLAPIDataSource() {
        this.locale = 'en-US';
        this.id = 'container';
        this.connectClick = $.proxy(this.connectDataSource, this);
        this.renderErrorToolTip(this.id);
    }
    CosmosDBSQLAPIDataSource.prototype.renderConfig = function (targetTag, dataSource, isEdit) {
        this.renderConfiguration(targetTag);
        this.updateDataSource(dataSource);
    };
    CosmosDBSQLAPIDataSource.prototype.renderConfiguration = function (targetTag) {
    ...
    CosmosDBSQLAPIDataSource.prototype.renderOdataPanel = function () {
    ...
    CosmosDBSQLAPIDataSource.prototype.updateDataSource = function (dataSource) {
        if (!ej.isNullOrUndefined(dataSource)) {
            this.connString.val(dataSource.ConnectionProperties.ConnectString);
        }
    };
    CosmosDBSQLAPIDataSource.prototype.connectDataSource = function (args) {
        if (!args[0].isCancel) {
            args[0].data = this.getDatasourceInfo(args[0].name);
        }
    };
    CosmosDBSQLAPIDataSource.prototype.showConfiguration = function (isShow) {
        this.odataConfig.css('display', isShow ? 'table-row' : 'none');
    };
    CosmosDBSQLAPIDataSource.prototype.resetInputFields = function () {
        this.connString.val('');
        this.hideValidationMsg();
    };
    CosmosDBSQLAPIDataSource.prototype.getDatasourceInfo = function (dataSourceName, dataSource) {
        var isValidCon = true;
        ...
        if (isValidCon) {
			...
			reportData.ConnectionProperties.IntegratedSecurity=true;
            reportData.ConnectionProperties.UserName = 'asdf';
            reportData.ConnectionProperties.PassWord = 'asdf';
            reportData.ConnectionProperties.DataProvider = 'CosmosDBSQLAPI';
            reportData.ConnectionProperties.ConnectString = this.connString.val();
            return reportData;
        }
        return null;
    };
    CosmosDBSQLAPIDataSource.prototype.createDataSource = function () {
        var dataSource = {
    ...
    CosmosDBSQLAPIDataSource.prototype.getLocale = function (text) {
        var odataLocale;
        var defaultLocale = CosmosDBSQLAPIDataSource.Locale['en-US'];
        if (!ej.isNullOrUndefined(CosmosDBSQLAPIDataSource.Locale[this.locale])) {
            odataLocale = CosmosDBSQLAPIDataSource.Locale[this.locale];
        }
    ...
    CosmosDBSQLAPIDataSource.prototype.updateCulture = function (culture) {
        this.locale = culture;
        if (this.connString) {
    ...
    CosmosDBSQLAPIDataSource.prototype.renderTextArea = function (name, id, target, value) {
    ...
    CosmosDBSQLAPIDataSource.prototype.getRowCaption = function (caption, id) {
    ...
    CosmosDBSQLAPIDataSource.prototype.updateRow = function (target, id, text) {
        target.find('#' + id + '_tr .e-designer-title-label').html(text);
    };
    CosmosDBSQLAPIDataSource.prototype.renderErrIndictor = function (target, ctrlId) {
    ...
    CosmosDBSQLAPIDataSource.prototype.showErrIndictor = function (target, isEnable, errMsg) {
        var errorIcon = target.find('.e-error-tip');
        errorIcon.attr('e-errormsg', errMsg).css('display', isEnable ? 'block' : 'none');
    };
    CosmosDBSQLAPIDataSource.prototype.renderErrorToolTip = function (id) {
    ...
    CosmosDBSQLAPIDataSource.prototype.showErrTip = function (ctrlId, args) {
        args.preventDefault();
    ...
    CosmosDBSQLAPIDataSource.prototype.hideErrTip = function (ctrlId, args) {
        $('#' + ctrlId + '_error_tooltip').css('display', 'none');
    };
    CosmosDBSQLAPIDataSource.prototype.showValidationMsg = function (id, isShow, msg) {
    ...
    CosmosDBSQLAPIDataSource.prototype.hideValidationMsg = function () {
        this.showValidationMsg(this.connString.attr('id'), false);
    };
    CosmosDBSQLAPIDataSource.prototype.updateValidationMsg = function (target, id, msg) {
    ...
    CosmosDBSQLAPIDataSource.prototype.dispose = function () {
    ...
    CosmosDBSQLAPIDataSource.prototype.destroyEjObjects = function (ejObjects, isRootEle) {
    ...
    return CosmosDBSQLAPIDataSource;
}());
CosmosDBSQLAPIDataSource.Locale = {};
CosmosDBSQLAPIDataSource.Locale['en-US'] = {
    connectionString: 'Connection String',
    alertMessage: 'Specify the Connection string'
};
CosmosDBSQLAPIDataSource.Locale['fr-FR'] = {
